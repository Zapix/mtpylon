# -*- coding: utf-8 -*-
import pytest


from mtpylon.crypto import AuthKey, AuthKeyManager, AuthKeyDoesNotExist

auth_key_tests = [
    (
        ,  # noqa
        0x189e4dd9d40b7e7b5c160c4b0313e843b05b5983,
        0x0313e843b05b5983,
        0x189e4dd9d40b7e7b,
    ),
    (
        ,  # noqa
        0xce10fe6e8e29cffb284ed5662f8ba7d92e35cfa3,
        0x2f8ba7d92e35cfa3,
        0xce10fe6e8e29cffb,
    ),
    (
        ,  # noqa
        0x03aa5f90a079852b27281e57fb8a0a8d4da8d551,
        0xfb8a0a8d4da8d551,
        0x03aa5f90a079852b,
    )
]


@pytest.mark.asyncio
@pytest.mark.parametrize(
    'auth_key,auth_hash,auth_key_id,auth_aux_hash',
    auth_key_tests
)
async def test_set_auth_key(auth_key, auth_hash, auth_key_id, auth_aux_hash):
    manager = AuthKeyManager()

    assert not await manager.has_key(auth_key_id)

    await manager.set_key(auth_key)

    assert await manager.has_key(auth_key_id)


@pytest.mark.asyncio
@pytest.mark.parametrize(
    'auth_key',
    [item[0] for item in auth_key_tests]
)
async def test_has_auth_key_data(auth_key):
    manager = AuthKeyManager()

    auth_key_data = AuthKey(auth_key)
    await manager.set_key(auth_key_data)

    assert await manager.has_key(auth_key_data)


@pytest.mark.asyncio
@pytest.mark.parametrize(
    'auth_key_id',
    [
        0x0313e843b05b5983,
        0x2f8ba7d92e35cfa3,
        0xfb8a0a8d4da8d551,
    ]
)
async def test_auth_key_get_error(auth_key_id):
    manager = AuthKeyManager()

    with pytest.raises(AuthKeyDoesNotExist):
        await manager.get_key(auth_key_id)


@pytest.mark.asyncio
@pytest.mark.parametrize(
    'auth_key,auth_hash,auth_key_id,auth_aux_hash',
    auth_key_tests
)
async def test_auth_key_get(auth_key, auth_hash, auth_key_id, auth_aux_hash):
    manager = AuthKeyManager()
    await manager.set_key(auth_key)

    auth_key_data = await manager.get_key(auth_key_id)

    assert auth_key_data.id == auth_key_id
    assert auth_key_data.value == auth_key
    assert auth_key_data.hash == auth_hash
    assert auth_key_data.aux_hash == auth_aux_hash


@pytest.mark.asyncio
@pytest.mark.parametrize(
    'auth_key_id',
    [
        0x0313e843b05b5983,
        0x2f8ba7d92e35cfa3,
        0xfb8a0a8d4da8d551,
    ]
)
async def test_auth_key_del_error(auth_key_id):
    manager = AuthKeyManager()

    with pytest.raises(AuthKeyDoesNotExist):
        await manager.del_key(auth_key_id)


@pytest.mark.asyncio
@pytest.mark.parametrize(
    'auth_key,auth_key_id',
    [(item[0], item[2]) for item in auth_key_tests]
)
async def test_auth_key_del_by_id(auth_key, auth_key_id):
    manager = AuthKeyManager()

    await manager.set_key(auth_key)
    assert await manager.has_key(auth_key_id)
    await manager.del_key(auth_key_id)

    assert not await manager.has_key(auth_key_id)


@pytest.mark.asyncio
@pytest.mark.parametrize(
    'auth_key,auth_key_id',
    [(item[0], item[2]) for item in auth_key_tests]
)
async def test_auth_key_del_by_auth_data(auth_key, auth_key_id):
    manager = AuthKeyManager()

    await manager.set_key(auth_key)

    auth_key_data = await manager.get_key(auth_key_id)
    await manager.del_key(auth_key_data)

    assert not await manager.has_key(auth_key_data)
